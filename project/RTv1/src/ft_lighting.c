...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/17 15:31:58 by krakharo          #+#    #+#             */
/*   Updated: 2019/07/06 13:15:22 by dhorker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_rtv1.h"

int		ft_shadow(t_th *mlx, t_obj *tmp, t_obj *light, t_vec pos)
{
	t_obj	*node;
	t_vec	dist;
...
t_obj	*ft_ref_inter(t_th *mlx, t_obj *node, t_obj *tmp, t_vec pos)
{
	t_obj	*tmp2;
	double	dist;
...
t_obj	*ft_ref_init(t_th *mlx, t_obj *tmp, t_vec *pos)
{
	t_obj	*tmp2;
...
double	*ft_reflect(t_th *mlx, t_obj *tmp, t_vec *pos, double *tab)
{
	t_obj	*tmp2;
	t_obj	*light;
	t_vec	dist;
	...
	return ((mlx->pref == 1 && tmp2->type != 3) ? \
			tab : ft_reflect(mlx, tmp2, pos, tab));
}

double	*ft_lambert(t_th *mlx, t_obj *tmp, t_obj *light, double *tab)
{
	t_vec	pos;
	t_vec	dist;
	double	d;
	...
	return ((mlx->pref == 1 && tmp->type != 3) ? \
			tab : ft_reflect(mlx, tmp, &pos, tab));
}
